  ...
  useTask$(({ track }) => {
    ...
    if (mapContainerRef.value && !mapInitialized.value) {
      import('leaflet').then((L) => {
        const lat = -7.0168872142804535;
        const long = 110.38560231867122;

        const map = L.map(mapContainerRef.value as HTMLElement).setView([lat, long], 20);
        ...
        const marker = L.marker([lat, long], { icon: qwikMarker }).addTo(map);
